...
IPAddress ip_conf(192, 168, 0, 1);
IPAddress mask_conf(255, 255, 0, 0);
...
{
    webInterface.serveStatic("/bootstrap.min.css", SPIFFS, "/bootstrap.min.css");
    if (getApMode())
    {
    ...
    {
        webInterface.serveStatic("/", SPIFFS, "/index.html");
    }

    webInterface.serveStatic("/ap_conf", SPIFFS, "/ap_conf.html");
    webInterface.on("/ap_conf_submit", handleConfSubmit);

    //webInterface.serveStatic("/ap_conf_reboot.html", SPIFFS, "/ap_conf_reboot.html");

    httpUpdater.setup(&webInterface);
    webInterface.begin();
}
...
void handleConfSubmit()
{
    ...
    /*webInterface.sendHeader("Location", "/ap_conf_reboot.html", true);
    webInterface.send(302, "text/plane", "");*/
    struct APConf ap;
